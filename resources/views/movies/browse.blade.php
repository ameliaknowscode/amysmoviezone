    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <form method="GET" action="{{ route('movies.browse') }}"
                  class="bg-white shadow-sm sm:rounded-lg px-4 py-4 flex flex-wrap gap-3 items-end">
                ...
                <div class="flex items-end gap-2">
                    ...
                    @if($hasFilters)
                        <a href="{{ route('movies.browse') }}"
                           class="px-4 py-2 bg-gray-100 text-gray-600 text-sm font-medium rounded-md hover:bg-gray-200 transition-colors">
                            Clear
                        </a>
                    @endif
                </div>
            ...
            {{-- Results --}}
            @if($movies->isEmpty())
                <p class="text-sm text-gray-500 px-1">No movies matched your filters.</p>
            @else
                @if($hasFilters)
