<nav x-data="{ open: false, searchOpen: false }" aria-label="Main" class="bg-zinc-950 border-b border-zinc-800">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('home') }}" class="flex items-center gap-2">
                        <x-application-logo class="block h-9 w-auto fill-current text-zinc-400" />
                        <span class="text-sm font-semibold text-zinc-100">{{ config('app.name') }}</span>
                    </a>
                </div>
                        ...
                        <x-dropdown align="left" width="48">
                            <x-slot name="trigger">
                                <button type="button"
                                        ...
                                        aria-haspopup="menu"
                                        class="inline-flex items-center px-1 pt-1 border-b-2 {{ request()->routeIs('movies.browse', 'recommendations', 'director-connections', 'compare.*', 'collections.public.*') ? 'border-amber-400 text-amber-400' : 'border-transparent text-zinc-400 hover:text-zinc-100 hover:border-zinc-600' }} text-sm font-medium leading-5 focus:outline-none transition duration-150 ease-in-out">
                                    {{ __('Explore') }}
                            ...
                            <x-slot name="content">
                                ...
                                <x-dropdown-link :href="route('collections.public.index')">
                                    {{ __('Collections') }}
                                </x-dropdown-link>
                                @auth
                    ...
                    <x-dropdown align="left" width="48">
                        <x-slot name="trigger">
                            <button type="button"
                                    :aria-expanded="open ? 'true' : 'false'"
                                    aria-haspopup="menu"
                                    class="inline-flex items-center px-1 pt-1 border-b-2 {{ request()->routeIs('admin.*', 'dashboard') ? 'border-amber-400 text-amber-400' : 'border-transparent text-zinc-400 hover:text-zinc-100 hover:border-zinc-600' }} text-sm font-medium leading-5 focus:outline-none transition duration-150 ease-in-out">
                                {{ __('Admin') }}
                                <svg class="ms-1 fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" aria-hidden="true">
                                    <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
                                </svg>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6 gap-2">
                <button
                    ...
                    class="inline-flex items-center gap-1 px-3 py-2 text-sm text-zinc-400 hover:text-zinc-300 focus:outline-none transition"
                    aria-label="Search"
                >
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        ...
                        <button type="button"
                                :aria-expanded="open ? 'true' : 'false'"
                                aria-haspopup="menu"
                                aria-label="Account menu{{ $unreadCount > 0 ? ', ' . $unreadCount . ' unread notification' . ($unreadCount === 1 ? '' : 's') : '' }}"
                                class="inline-flex items-center gap-2 px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-zinc-400 hover:text-zinc-200 focus:outline-none transition ease-in-out duration-150">
                            <div class="relative">
                                @if(Auth::user()->avatar)
                                    <img src="{{ asset('storage/' . Auth::user()->avatar) }}" alt="{{ Auth::user()->name }}" class="h-7 w-7 rounded-full object-cover">
                                @else
                                    <div class="h-7 w-7 rounded-full bg-zinc-800 flex items-center justify-center text-zinc-300 text-xs font-bold select-none">
                                        {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                                    </div>
                                @endif
                                @if($unreadCount > 0)
                                    <span aria-hidden="true" class="absolute -top-1 -right-1 h-3.5 w-3.5 rounded-full bg-red-500 border-2 border-zinc-950 flex items-center justify-center">
                ...
                @else
                <a href="{{ route('register') }}" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md bg-amber-500 hover:bg-amber-400 text-zinc-950 font-semibold focus:outline-none transition ease-in-out duration-150">
                    {{ __('Sign Up') }}
                </a>
                <a href="{{ route('login') }}" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-zinc-400 hover:text-zinc-200 focus:outline-none transition ease-in-out duration-150">
                    {{ __('Log In') }}
                </a>
                @endauth
            </div>
            ...
            <div class="-me-2 flex items-center sm:hidden">
                <button @click="open = ! open"
                        ...
                        class="inline-flex items-center justify-center p-2 rounded-md text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 focus:outline-none focus:bg-zinc-800 transition duration-150 ease-in-out">
    ...
    >
        <div
            @click.stop
            x-transition.scale
            class="bg-zinc-900 border border-zinc-700 rounded-lg shadow-xl p-6 w-full max-w-md mx-4"
        >
            <h3 id="search-dialog-title" class="text-lg font-semibold text-zinc-100 mb-4">Search movies &amp; people</h3>

            <form method="GET" action="{{ route('search') }}">
                <div class="flex items-center gap-2">
                    <label for="nav-search-input" class="sr-only">Search movies and people</label>
                    <input
                        id="nav-search-input"
                        type="text"
                        name="q"
                        ...
                        class="flex-1 bg-zinc-800 border border-zinc-700 rounded-md px-3 py-2 text-sm text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-amber-500"
                    />
                    <button
                        type="submit"
                        class="inline-flex items-center gap-1 px-4 py-2 bg-amber-500 hover:bg-amber-400 text-zinc-950 text-sm font-semibold rounded-md transition"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-4.35-4.35M17 11A6 6 0 1 1 5 11a6 6 0 0 1 12 0z" />
                        </svg>
            ...
            <div class="mt-4 text-right">
                <button @click="searchOpen = false" class="text-sm text-zinc-400 hover:text-zinc-300">Cancel</button>
            </div>
        </div>
    </div>
    ...
    <div id="mobile-nav" :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <button
                ...
                class="w-full text-left block ps-3 pe-4 py-2 border-l-4 border-transparent text-base font-medium text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 hover:border-zinc-600 focus:outline-none transition"
            >
                Search
            </button>
            <div class="px-4 pt-2 pb-1 text-xs font-semibold uppercase tracking-widest text-zinc-400">Explore</div>
            <x-responsive-nav-link :href="route('movies.browse')" :active="request()->routeIs('movies.browse')">
                {{ __('Movies') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('collections.public.index')" :active="request()->routeIs('collections.public.*')">
                {{ __('Collections') }}
            </x-responsive-nav-link>
            @auth
            ...
            @auth
            <div class="px-4 pt-3 pb-1 text-xs font-semibold uppercase tracking-widest text-zinc-400">My Zone</div>
        ...
        <div class="pt-4 pb-1 border-t border-zinc-800">
            ...
            <div class="px-4 flex items-center gap-3">
                ...
                @else
                    <div class="h-10 w-10 rounded-full bg-zinc-800 flex items-center justify-center text-zinc-300 text-sm font-bold select-none shrink-0">
                        {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                    </div>
                @endif
                <div>
