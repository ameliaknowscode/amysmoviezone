                ...
                <table class="min-w-full divide-y divide-zinc-700 text-sm">
                    <thead class="bg-zinc-900">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-zinc-400 uppercase tracking-wider">Slug</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-zinc-400 uppercase tracking-wider">Films</th>
                    ...
                    <tbody class="divide-y divide-zinc-800">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-3 text-zinc-400">{{ $collection->slug }}</td>
                            <td class="px-6 py-3 text-zinc-400">{{ $collection->movies_count }}</td>
                            <td class="px-6 py-3 text-right space-x-3">
                                <a href="{{ route('admin.collections.edit', $collection) }}"
                                   class="text-zinc-400 hover:text-amber-400 transition-colors" title="Edit">
                                ...
                                <form method="POST" action="{{ route('admin.collections.destroy', $collection) }}"
                                    ...
                                    @csrf @method('DELETE')
                                    <button type="submit" class="text-zinc-400 hover:text-red-600 transition-colors" title="Delete">
                                        <svg class="inline w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        @empty
                        <tr>
                            <td colspan="4" class="px-6 py-8 text-center text-zinc-400">No collections yet.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
