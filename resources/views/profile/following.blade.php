<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-zinc-100 leading-tight">
            <a href="{{ route('profile.show', $profileUser->username) }}" class="hover:text-amber-400 transition-colors">{{ $profileUser->name }}</a> is Following
        </h2>
    </x-slot>
            ...
            <div class="card">
                <div class="p-6">
                    @if($following->isEmpty())
                        <p class="text-sm text-zinc-400">Not following anyone yet.</p>
                    @else
