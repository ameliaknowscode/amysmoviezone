    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <form method="POST" action="{{ route('lists.update', $movieList) }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3" maxlength="1000"
                                  class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm">{{ old('description', $movieList->description) }}</textarea>
                    </div>
                    ...
                    <div class="flex items-center gap-3 pt-1">
                        ...
                        <a href="{{ route('lists.show', $movieList) }}" class="text-sm text-gray-500 hover:text-gray-700">Cancel</a>
                    </div>
                </form>
            </div>
                ...
                <div class="bg-white shadow-sm sm:rounded-lg p-6"
                    ...
                    <h3 class="text-sm font-semibold text-gray-700 mb-4">
                        ...
                        @if($movieList->is_ranked)
                            <span class="text-gray-400 font-normal ml-1">— drag to reorder</span>
                        @else
                            <span class="text-gray-400 font-normal ml-1">— drag to reorder</span>
                        @endif
                    </h3>
                    ...
                    <ul id="sortable-list" class="space-y-2">
                        @foreach($movieList->items as $item)
                            <li class="flex items-center gap-3 bg-gray-50 rounded-lg px-3 py-2 cursor-grab active:cursor-grabbing"
                                data-id="{{ $item->id }}">
                                {{-- Drag handle --}}
                                ...
                                @if($movieList->is_ranked)
                                    <span class="rank-number text-xs font-bold text-gray-400 w-5 text-center shrink-0">{{ $loop->iteration }}</span>
                                @endif

                                <div class="w-8 h-12 bg-gray-200 rounded overflow-hidden shrink-0">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-medium text-gray-800 truncate">{{ $item->movie->title }}</p>
                                    @if($item->movie->release_year)
                                        <p class="text-xs text-gray-400">{{ $item->movie->release_year }}</p>
                                    @endif
                                </div>
                                ...
                                <form method="POST"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-gray-300 hover:text-red-400 transition-colors p-1" aria-label="Remove {{ $item->movie->title }} from list">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6 border border-red-100">
                ...
                <p class="text-sm text-gray-500 mb-4">This will permanently delete the list and all its items.</p>
